...
(function (root, factory) {
  ...
  /**
   * Ascender extension — the `height` handle for ascender letters
   * (b/d/f/h/k/l/t).
   *
   * Lengthens ONLY the ascender: the part of the outline ABOVE the x-height
   * line. The x-height body and the baseline stay fixed, so this is a true
   * ascender control distinct from the x-height band scale (bandScaleY). The
   * pivot is the x-height line `xHeightTopY` (a negative y, up = −y): points at
   * or below it are pinned, points above it scale their distance above the line
   * by `f`. `f = 1` is identity.
   */
  function ascenderScaleY(commands, f, xHeightTopY) {
    if (!commands || Math.abs(f - 1) < 0.001) return commands;
    return transformCommandPoints(commands, function (x, y) {
      // y strictly above the x-height line (more negative) → scale; the
      // x-height body / baseline / any descender at or below stays put.
      if (y < xHeightTopY) {
        return { x, y: xHeightTopY + (y - xHeightTopY) * f };
      }
      return { x, y };
    });
  }
...
(function () {
  ...
  class DeformableOutlineWordmark {
    constructor(text, options) {
      ...
      // Set once delegated glyph listeners are bound in mount (CL8).
      this._glyphEventsBound = false;
      this._layoutCache = null;
    ...
    // Re-deform glyphs from their per-letter axisValues. Pass `onlyIndex` to
    // re-deform a single glyph (the drag hot path only touches the dragged
    // letter — CL8); omit it for master gestures (sliders, reset, mouse-follow)
    // that change every letter.
    _applyDeformToGlyphs(onlyIndex) {
      const preset = presets[this.presetKey];
      const deform = (g) => {
        if (!g || g.fallback === "parametric" || !g._baseGlyph) return;
      ...
      if (onlyIndex != null) {
        deform(this.glyphs[onlyIndex]);
        return;
      }
      for (const g of this.glyphs) deform(g);
    }
    ...
    setAxis(id, value) {
      ...
      // _loadGlyphs always seeds g.axisValues, so no lazy-init guard needed.
    ...
    mount(target) {
      ...
      this.svgEl.appendChild(this.handleLayer);
      this.svgEl.appendChild(this.tooltipLayer);
      this._bindGlyphLayerEvents();
      describeWordmarkSvg(this.svgEl, this.text, this.interactive);
      this._render();
      return this.svgEl;
    }

    // Delegate glyph interaction to ONE set of listeners on the persistent
    // glyphLayer (added once at mount) instead of re-binding N listeners on
    // every _render (CL8). pointerdown/keydown bubble; focusin/focusout are
    // the bubbling counterparts of focus/blur. Each resolves the glyph node
    // via closest('[data-glyph-index]'), so non-interactive fallback glyphs
    // (which lack the attribute) are ignored.
    _bindGlyphLayerEvents() {
      if (!this.glyphLayer || this._glyphEventsBound) return;
      this._glyphEventsBound = true;
      const nodeFor = (e) => {
        const t = e.target;
        return t && t.closest ? t.closest("[data-glyph-index]") : null;
      };
      this.glyphLayer.addEventListener("pointerdown", (e) => {
        if (!this.interactive) return;
        if (nodeFor(e)) this._onGlyphDragStart(e);
      });
      this.glyphLayer.addEventListener("keydown", (e) => {
        if (!this.interactive) return;
        if (nodeFor(e)) this._onGlyphKeyDown(e);
      });
      this.glyphLayer.addEventListener("focusin", (e) => {
        const node = nodeFor(e);
        if (node)
          this._focusedGlyphIndex = parseInt(node.dataset.glyphIndex, 10);
      });
      this.glyphLayer.addEventListener("focusout", () => {
        this._focusedGlyphIndex = null;
      });
    }
    ...
    _render() {
      ...
      // node fires focusout (delegated on glyphLayer), which nulls
      // _focusedGlyphIndex, so read it first. Glyph interaction is handled by
      // the delegated listeners bound once in mount (CL8), so there are no
      // per-glyph listeners to re-attach here.
      const focusIndex = this._focusedGlyphIndex;
      this.glyphLayer.innerHTML = parts.join("");
      if (this.interactive && focusIndex != null) {
    ...
    _renderGlyphTooltip(index, layout) {
      ...
      const fontSizePx = 22;
      // Real rendered metrics (CL11) instead of a fixed per-char estimate so
      // multi-axis text doesn't overflow/underfill the bubble.
      const textW = measureSvgText(this.tooltipLayer, text, fontSizePx, {
        fontFamily: MONO,
      });
    ...
    // Convert client coords to SVG-local. `invMatrix` lets a drag pass its
    // cached inverse screen CTM (constant during a drag) so each pointermove
    // skips a getScreenCTM()/inverse() (CL8).
    _toSvgLocal(client, invMatrix) {
      const pt = this.svgEl.createSVGPoint();
      pt.x = client.x;
      pt.y = client.y;
      const inv = invMatrix || this.svgEl.getScreenCTM().inverse();
      const screen = pt.matrixTransform(inv);
    ...
    _onGlyphDragStart(e) {
      if (!this.interactive) return;
      // Delegated from glyphLayer, so resolve the glyph node from the target.
      const node = e.target && e.target.closest("[data-glyph-index]");
      if (!node) return;
      const index = parseInt(node.dataset.glyphIndex, 10);
      const g = this.glyphs[index];
      if (!g) return;
      const bubbleAxis =
      ...
      // Cache the inverse screen CTM once — it's constant for the whole drag,
      // so each pointermove reuses it instead of recomputing getScreenCTM (CL8).
      let invMatrix = null;
      try {
        const ctm = this.svgEl.getScreenCTM();
        if (ctm) invMatrix = ctm.inverse();
      } catch {}
      const local = this._toSvgLocal({ x: e.clientX, y: e.clientY }, invMatrix);
      this.dragState = {
        kind: "glyph",
        index,
        startX: local.x,
        ...
        // Inverse screen CTM cached for the duration of the drag (CL8).
        invMatrix,
      };
    ...
    _onGlyphDragMove(e) {
      ...
      // Only the dragged glyph changed — re-deform just that one (CL8).
      this._applyDeformToGlyphs(ds.index);
      this._render();
    }
    ...
    _onGlyphDragEnd() {
      ...
      // The final glyph frame was already painted by the last _onGlyphDragMove;
      // the only drag-state-dependent visual is the value tooltip, so just
      // clear it instead of a full re-render (CL13).
      if (this.tooltipLayer) this.tooltipLayer.innerHTML = "";
    }
    ...
    disableMouseFollow() {
      ...
      window.removeEventListener("mousemove", this._mouseMoveBound);
      document.removeEventListener("mouseleave", this._mouseLeaveBound);
      const rest = Object.assign({}, this._mouseFollow.restAxes);
      this.axisValues = rest;
      // Mouse-follow broadcast its values onto every glyph's axisValues, so the
      // rest snapshot must be broadcast back too — otherwise glyphs keep the
      // last cursor-driven values. (Consistent with the master model: enabling
      // mouse-follow overrides per-letter drags.)
      for (const g of this.glyphs) {
        if (g.fallback === "parametric" || !g.axisValues) continue;
        for (const ax of this.presetAxes) {
          if (rest[ax.id] != null) g.axisValues[ax.id] = rest[ax.id];
        }
      }
      this._mouseFollow = null;
      this._applyDeformToGlyphs();
...
(function () {
  ...
  class AnatomyDeformWordmark {
    ...
    // Bounding box of a glyph AS DEFORMED, so the layout/viewBox reflect the
    // live silhouette rather than the undeformed font box. `_resolveGlyphPath`
    // is memoised per handle-state (and is called again in _glyphMarkup this
    // same render, a cache hit), so this is cheap. Falls back to the static
    // bounds for glyphs with no resolved commands (space / load failure).
    _liveGlyphBounds(g) {
      const resolved = this._resolveGlyphPath(g);
      const cmds = resolved && resolved.cmds;
      if (!cmds || !cmds.length) return g.bounds;
      const b = boundsFromCommands(cmds);
      // descenderDepth is a wrap-group `scale(1 dd)` pivoted at the baseline
      // (y=0), applied AFTER cmds in _glyphMarkup, so it never reaches `cmds`.
      // Fold it into the below-baseline extent the same way the render does.
      ...
      const dd =
        isDescender && g.handleState.descenderDepth != null
          ...
          : 1;
      const maxY = b.maxY > 0 ? b.maxY * dd : b.maxY;
      return { minX: b.minX, maxX: b.maxX, minY: b.minY, maxY };
    }
    ...
    _layout() {
      ...
      const placed = this.glyphs.map((glyph) => {
        ...
        // Use the LIVE deformed extent, not the static font bbox, so the
        // viewBox grows when the height/ascender/descender handles lengthen a
        // glyph past its original box (otherwise the SVG crops them — CL).
        const b = this._liveGlyphBounds(glyph);
        const ascent = Math.max(0, -b.minY);
        const descent = Math.max(0, b.maxY);
        if (ascent > maxAscent) maxAscent = ascent;
        if (descent > maxDescent) maxDescent = descent;
    ...
    _resolveGlyphPath(g) {
      ...
      try {
        ...
        if (heightActive) {
          // Ascender letters: the handle is labelled "ascender" and lengthens
          // the stem above x-height (x-height body stays put). Everyone else:
          // scale the body up to its x-height / cap-height band.
          cmds = ANATOMY_ASCENDER_LETTERS.has(g.character)
            ? ascenderScaleY(cmds, height, this._xHeightTopY())
            : bandScaleY(cmds, height, this._bandTopYFor(g.character));
        }
        if (widthActive) {
    ...
    // Band top for the height handle's region-clipped scale (NON-ascender
    // letters): uppercase clips to cap-height (whole cap scales, baseline
    // pinned); everything else clips to x-height so the body scales while any
    // ascender/descender above the line stays put. Ascender letters
    // (b/d/f/h/k/l/t) do NOT come through here — their "ascender" handle uses
    // ascenderScaleY to lengthen the stem above x-height (see _resolveGlyphPath).
    // Returned as a negative y (glyph space has up = −y). Falls back to a
    // generous band if metrics never loaded.
    ...
    // The x-height line (negative y) used as the pivot for the ascender handle.
    _xHeightTopY() {
      const m = this._bandMetrics || {};
      const xH = m.xHeight || this.fontSize * 0.5;
      return -xH;
