...
(function (root, factory) {
  ...
  // group so the focusable handle buttons inside stay exposed to AT (role
  // "img" would collapse the SVG into one opaque image and hide them).
      ...
      if (this.interactive && this.handleLayer) {
        ...
        axisHandles.forEach((h) => {
          ...
          html.push(
            '<circle data-axis-handle="true" data-handle-id="' +
              ...
              '" r="' +
              targetRadius +
              // Cursor via inline style (the SVG `cursor`
              // presentation attribute isn't reliably honored). The bubbly
              // axis sliders move vertically, so ns-resize reads the zone.
              '" fill="#000" fill-opacity="0.001" stroke="none" style="cursor:ns-resize;touch-action:none"/>'
          );
          html.push(
            '<circle cx="' +
              h.control.x +
        ...
        this.handleLayer
          ...
          .forEach((c) => {
            ...
            if (!ax) return;
            const label = this._humanize(handleId) + " axis control";
            makeHandleKeyboardOperable(c, label, (dir, mult) => {
              const step = ((ax.max - ax.min) / 100) * mult;
              this.setAxis(handleId, this.axisValues[handleId] + dir * step);
              const next = this.handleLayer.querySelector(
                'circle[data-axis-handle][data-handle-id="' + handleId + '"]'
              );
              if (next) next.focus();
            });
          });
